...
@section('title', 'Tambah Detail Tindakan/Terapi | RSHP Unair')

@section('content')
<div class="mb-4">
    <h3 class="fw-bold text-primary"><i class="bi bi-plus-circle"></i> Tambah Detail Tindakan/Terapi</h3>
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        <form action="{{ route('detailrekammedis.store') }}" method="POST">
            @csrf
                ...
                <select class="form-select @error('idkode_tindakan_terapi') is-invalid @enderror" 
                    ...
                    @foreach($kodeTindakanTerapi as $ktt)
                        <option value="{{ $ktt->idkode_tindakan_terapi }}" 
                                ...
                                {{ old('idkode_tindakan_terapi') == $ktt->idkode_tindakan_terapi ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('detail') is-invalid @enderror" 
                          ...
                          name="detail" 
                          rows="4"
                          placeholder="Contoh: Dosis, frekuensi pemberian, hasil pemeriksaan, dll...">{{ old('detail') }}</textarea>
                @error('detail')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
