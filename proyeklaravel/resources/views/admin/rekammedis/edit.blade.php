...
@section('content')
<div class="mb-4">
    <h3 class="fw-bold text-primary"><i class="bi bi-pencil-square"></i> Edit Rekam Medis</h3>
...
<div class="card border-0 shadow-sm rounded-4">
    <div class="card-body p-4">
        <form action="{{ route('rekammedis.update', $rekamMedis->idrekam_medis) }}" method="POST">
            @csrf
            @method('PUT')

            <!-- ID (Read-only) -->
            <div class="mb-3">
                <label for="id" class="form-label fw-semibold">ID Rekam Medis</label>
                <input type="text" 
                       class="form-control" 
                       id="id" 
                       value="{{ $rekamMedis->idrekam_medis }}"
                       disabled>
                    ...
                    <select class="form-select @error('dokter_pemeriksa') is-invalid @enderror" 
                        ...
                        @foreach($dokters as $dokter)
                            <option value="{{ $dokter->idrole_user }}" 
                                    {{ old('dokter_pemeriksa', $rekamMedis->dokter_pemeriksa) == $dokter->idrole_user ? 'selected' : '' }}>
